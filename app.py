            ...
            html.Label([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[
                            {
                                'label': i, 'value': i
                            } for i in df.columns if i not in ['gas_leaks','gas_leaks_per_person','geoid', 'incident_year','boro', 'nta', 'centerLong', 'centerLat', 'area', 'polygon', 'hover']
                        ],
                        value=['lonely_housholder%', 'not_us_citizen%','mean_houshold_income'],
                        multi=True,
...
'''        row9 with pie chart (property use)
        html.Div([
            dcc.Graph(
...
def build_parallel_coord(filtered_geoids, year, toggle, limit):
    ...
    # array of attributes
    arr1 = [str(r) for r in columns_original[3:] if r != 'gas_leaks_per_person' and r != 'gas_leaks']
    arr1 = sorted(arr1)
...
def display_selected_data(selected_geoids, hideOutliers, limit):
    ...
    df_pearson = df_selected.drop(
        ['geoid',  'centerLong', 'centerLat', 'total_housing_units','gas_leaks', 'area'], axis=1)
...
def display_selected_data(selected_geoids, hideOutliers, limit):
    ...
    for year in range(2013, 2018):
        ...
        df_year = df_selected[df_selected['incident_year'] == year].groupby(['incident_month']).agg(
            {'gas_leaks': 'sum', 'total_population': 'sum'}).reset_index()
        df_year['gas_leaks_per_person'] = df_year['gas_leaks']/df_year['total_population']*1000 # avoid tiny numbers and mu
    ...
    temp_df = df_selected.groupby(['incident_month']).agg(
        {'gas_leaks': 'sum', 'total_population': 'sum'}).reset_index()
    temp_df['gas_leaks_per_person'] = temp_df['gas_leaks']/temp_df['total_population']*1000 # avoid tiny numbers and mu
    ...
    fig.update_layout(xaxis_title='Month',
                      yaxis_title='Gas Leaks per Person (10<sup>-3</sup> gl/p)',
                      plot_bgcolor=colors['background'],
                      paper_bgcolor=colors['background'],
                      title={
